...
|                       | `KERNEL_PARAMS`     | Set Kernel specific params within ORTools. Only modify this if you know what you are doing as these values are heavily dependent on the underlying algorithm. An example of this would be `Threads 2\nMethod 3` for the GUROBI Kernel to set the number of threads it uses to 2 and the method to concurrent.     | _`Empty string`_                                                 |
