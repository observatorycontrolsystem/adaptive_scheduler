...
class OpenSearchSeeingMonitor(SeeingMonitor):
    ...
    def update_data(self):
        ...
        try:
            self.current_seeing_by_resource = get_seeing(
                self.get_resources(), self.opensearch_url, self.os_index, self.os_excluded_observatories, self.seeing_valid_time_period
            )
        except OSConnectionError as e:
            log.warning(f"Unable to get seeing data from opensearch: {repr(e)}")
            return False
