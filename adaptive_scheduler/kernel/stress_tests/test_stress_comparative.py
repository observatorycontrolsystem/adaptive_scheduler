...
from adaptive_scheduler.kernel.fullscheduler_v5 import FullScheduler_v5 as CurrentScheduler2
from util import *
...
class TestStress(object):
    ...
    def run_1(self, seed=None):
        ...
        tstart =  time()
        fs2 = CurrentScheduler2(cr_list, self.gpw, [], self.slice_dict)
        s2 = fs2.schedule_all()
        tend = time()
        u = Util()
        u.find_overlaps(s2)
        ...
        s = fs.schedule_all()
        tend2 = time()
        print len(s2['0']), sum(r.priority for r in s2['0']), tend-tstart, len(s['0']), sum(r.priority for r in s['0']), tend2-tend
        u.find_overlaps(s)
