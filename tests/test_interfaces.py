...
from adaptive_scheduler.interfaces import RunningRequestGroup, RunningRequest, ResourceUsageSnapshot
...
class TestResourceUsageSnapshot(object):
    ...
    def test_running_intervals(self):
        ...
        assert Intervals([]) == snapshot.running_intervals('1m0a.doma.lsc')
        assert (Intervals([{'time': start, 'type': 'start'}, {'time': end, 'type': 'end'}]) ==
    ...
    def test_running_requests_for_resources_returns_running_requests(self):
        ...
        assert 2 == len(running_requests)
        assert running_request1 in running_requests
        assert running_request2 in running_requests
